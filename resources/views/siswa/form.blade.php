			...
			@endif
				...
				{!! Form::date('tanggal_lahir', !empty($siswa) ? $siswa->tanggal_lahir->format('Y-m-d'): null, ['class' => 'form-control', 'id' => 'tanggal_lahir', 'placeholder' => 'yyyy-mm-dd']) !!}
			...
			@endif
				{!! Form::label('hobi_siswa', 'Hobi:', ['class'=>'control-label']) !!}
					@if (count($list_hobi) > 0)
						...
						<div class="checkbox"><label>{!! Form::checkbox('hobi_siswa[]', $key, null) !!}{{ $value }}</label></div>
						@endforeach
					@else
